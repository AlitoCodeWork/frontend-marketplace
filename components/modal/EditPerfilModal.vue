<template>
  <div class="fixed inset-0 bg-gray-500 bg-opacity-50 z-50 flex">
    <div class="min-h-screen bg-white w-full sm:w-[600px] md:w-[500px] lg:w-[400px] xl:w-[500px] rounded-lg p-6 max-h-[90vh] overflow-y-auto">
      <!-- Encabezado con flecha para cerrar -->
      <div class="flex items-center justify-center mt-4">
        <button @click="close" class="text-gray-600 hover:text-gray-800">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-8 h-8">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 12H5" />
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5l-7 7 7 7" />
          </svg>
        </button>
        <h2 class="text-xl font-semibold text-center">Editar Perfil de Negocio</h2>
      </div>

      <!-- Imagen actual y opción para cambiar -->
      <div class="flex justify-center mt-8">
        <div class="flex flex-col items-center">
          <!-- Mostrar imagen actual -->
          <div class="w-24 h-24 rounded-md overflow-hidden mb-4">
            <img
              :src="currentImageUrl"
              alt="Imagen actual"
              class="w-full h-full object-cover"
            />
          </div>
          
          <!-- Botón para cambiar imagen -->
          <label for="product-image" class="cursor-pointer bg-gray-100 p-2 rounded-md hover:bg-gray-200">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-6 h-6 text-gray-600">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            <input
              type="file"
              id="product-image"
              class="hidden"
              @change="handleImageChange"
              accept="image/*"
            />
          </label>
          <span class="text-xs text-gray-500 mt-1">Cambiar imagen</span>
        </div>
      </div>

      <!-- Formulario -->
      <form @submit.prevent="submitForm">
        <div class="mt-4">
          <label for="nombre_negocio" class="block text-sm font-medium text-gray-700">Nombre del negocio</label>
          <input
            type="text"
            id="nombre_negocio"
            v-model="form.nombre_negocio"
            class="mt-1 p-2 w-full border rounded-lg"
            placeholder="Escribe el nombre del negocio"
            required
          />
        </div>

        <div class="mt-4">
          <label for="descripcion" class="block text-sm font-medium text-gray-700">Descripción del negocio</label>
          <textarea
            id="descripcion"
            v-model="form.descripcion"
            class="mt-1 p-2 w-full border rounded-lg"
            placeholder="Describe brevemente tu negocio"
            required
          />
        </div>

        <div class="mt-4">
          <label for="direccion" class="block text-sm font-medium text-gray-700">Dirección del negocio</label>
          <input
            type="text"
            id="direccion"
            v-model="form.direccion"
            class="mt-1 p-2 w-full border rounded-lg"
            placeholder="Ingrese la dirección del negocio"
            required
          />
        </div>

        <!-- Datos de contacto -->
        <div class="mt-4">
          <label for="correo_contacto" class="block text-sm font-medium text-gray-700">Correo Electrónico de Contacto</label>
          <input
            type="email"
            id="correo_contacto"
            v-model="form.contacto_correo"
            class="mt-1 p-2 w-full border rounded-lg"
            placeholder="ej: negocio@correo.com"
            required
          />
        </div>

        <div class="mt-4">
          <label for="contacto_telefono" class="block text-sm font-medium text-gray-700">Teléfono</label>
          <input
            type="text"
            id="contacto_telefono"
            v-model="form.contacto_telefono"
            class="mt-1 p-2 w-full border rounded-lg"
            placeholder="ej: 0949954954"
            required
          />
        </div>

        <!-- Campos de latitud y longitud -->
        <div class="mt-4">
          <label for="latitud" class="block text-sm font-medium text-gray-700">Latitud</label>
          <input
            type="text"
            id="latitud"
            v-model="form.latitude"
            class="mt-1 p-2 w-full border rounded-lg"
            placeholder="Latitud"
            required
          />
        </div>

        <div class="mt-4">
          <label for="longitud" class="block text-sm font-medium text-gray-700">Longitud</label>
          <input
            type="text"
            id="longitud"
            v-model="form.longitude"
            class="mt-1 p-2 w-full border rounded-lg"
            placeholder="Longitud"
            required
          />
        </div>

        <!-- Botones de acción -->
        <div class="flex space-x-4 mt-8 mb-4">
          <button
            type="submit"
            class="bg-[#204347] text-white py-2 px-4 rounded-md flex-1 flex items-center justify-center space-x-2 hover:bg-[#16675c]"
            :disabled="loading"
          >
            <span v-if="loading">Actualizando...</span>
            <span v-else>Actualizar Perfil</span>
          </button>
          
          <button
            type="button"
            @click="close"
            class="bg-gray-500 text-white py-2 px-4 rounded-md flex-1 hover:bg-gray-600"
          >
            Cancelar
          </button>
        </div>
      </form>
    </div>
  </div>
</template>

<script setup>
import { reactive, ref, computed, onMounted } from 'vue'
import { useNuxtApp } from '#app'

const { $axios } = useNuxtApp()

// Props
const props = defineProps({
  perfil: {
    type: Object,
    required: true
  }
})

// Emits
const emit = defineEmits(['close'])

// Estado reactivo
const loading = ref(false)
const newImage = ref(null)

// Formulario reactivo
const form = reactive({
  nombre_negocio: '',
  direccion: '',
  descripcion: '',
  contacto_correo: '',
  contacto_telefono: '',
  longitude: '',
  latitude: '',
  image: null
})

// Computed para mostrar la imagen actual
const currentImageUrl = computed(() => {
  if (newImage.value) {
    return URL.createObjectURL(newImage.value)
  }
  return props.perfil?.imagen || '/placeholder.jpg'
})

// Función para cargar los datos del perfil en el formulario
const loadProfileData = () => {
  if (props.perfil) {
    form.nombre_negocio = props.perfil.nombre_negocio || ''
    form.direccion = props.perfil.direccion || ''
    form.descripcion = props.perfil.descripcion || ''
    form.contacto_correo = props.perfil.contacto_correo || ''
    form.contacto_telefono = props.perfil.contacto_telefono || ''
    form.longitude = props.perfil.longitude?.toString() || ''
    form.latitude = props.perfil.latitude?.toString() || ''
  }
}

// Función para manejar el cambio de imagen
const handleImageChange = (event) => {
  const file = event.target.files[0]
  if (file) {
    const allowedTypes = ['image/jpeg', 'image/png', 'image/jpg']
    if (!allowedTypes.includes(file.type)) {
      alert('Por favor, sube una imagen en formato JPEG, JPG o PNG')
      return
    }
    if (file.size > 5 * 1024 * 1024) { // Limitar el tamaño máximo a 5MB
      alert('La imagen no debe exceder los 5MB')
      return
    }
    newImage.value = file
    form.image = file
  }
}

// Función para enviar el formulario
const submitForm = async () => {
  try {
    loading.value = true
    
    // Obtener el token del localStorage
    const token = localStorage.getItem('token')
    if (!token) {
      alert('No hay token de autenticación')
      return
    }

    // Crear el FormData
    const formData = new FormData()
    formData.append('nombre_negocio', form.nombre_negocio)
    formData.append('direccion', form.direccion)
    formData.append('descripcion', form.descripcion)
    formData.append('contacto_correo', form.contacto_correo)
    formData.append('contacto_telefono', form.contacto_telefono)
    formData.append('longitude', form.longitude)
    formData.append('latitude', form.latitude)
    
    // Agregar imagen solo si se seleccionó una nueva
    if (form.image) {
      formData.append('imagen', form.image)
    }

    // Imprimir en consola los datos que se enviarán al backend
    console.log('Datos enviados al backend:', {
      nombre_negocio: form.nombre_negocio,
      direccion: form.direccion,
      descripcion: form.descripcion,
      contacto_correo: form.contacto_correo,
      contacto_telefono: form.contacto_telefono,
      longitude: form.longitude,
      latitude: form.latitude,
      imagen: form.image ? form.image.name : 'No se cambió la imagen'
    })

    // Enviar la solicitud PUT
    const response = await $axios.put('http://localhost:4000/vendedor/perfil', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
        'Authorization': `Bearer ${token}`
      }
    })

    alert('Perfil actualizado con éxito')
    emit('close') // Cerrar el modal después del éxito
    
  } catch (err) {
    console.error('Error al actualizar el perfil:', err)
    
    // Manejar errores específicos
    if (err.response?.status === 401) {
      alert('Token de autenticación inválido. Por favor, inicia sesión nuevamente.')
    } else if (err.response?.status === 400) {
      alert('Datos inválidos. Por favor, revisa el formulario.')
    } else {
      alert('Hubo un error al actualizar el perfil')
    }
  } finally {
    loading.value = false
  }
}

// Función para cerrar el modal
const close = () => {
  emit('close')
}

// Cargar los datos del perfil al montar el componente
onMounted(() => {
  loadProfileData()
})
</script>

<style scoped>
input[type="file"] {
  display: none;
}

button[type="submit"]:hover {
  background-color: #16675c;
}

button[type="submit"]:disabled {
  background-color: #9ca3af;
  cursor: not-allowed;
}
</style>